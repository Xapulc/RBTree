...
void RBTree::sumOneTier(const int tier, TreeNodeValue& sum) {
    sumOneTierHelper(*header.left, tier - 1, sum);
}

void RBTree::sumOneTierHelper(RBTreeNode& node, const int tier, TreeNodeValue& curSum) {
    assert(tier >= 0);
    if (tier == 0) {
        if (&node != 0)
            curSum += *node.value;
    } else {
        sumOneTierHelper(*node.left, tier - 1, curSum);
        sumOneTierHelper(*node.right, tier - 1, curSum);
    }
}